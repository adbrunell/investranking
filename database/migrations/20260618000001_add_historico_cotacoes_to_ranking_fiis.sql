...
WITH latest_prices AS (
  ...
  SELECT cnpj, sum(rendimento) AS total_rendimentos
  FROM fnet_tudo
  WHERE tipo = 'Dividendo' AND rendimento IS NOT NULL
    AND data_com >= CURRENT_DATE - INTERVAL '1 year'
  GROUP BY cnpj
), liquidez_30d AS (
...
LEFT JOIN dy_12m d ON d.cnpj = m.cnpj
