-- Make Ranking_FIIs read live prices directly from b3_cotacoes_aovivo
...
-- Update fn_refresh_ranking_fiis: now only refreshes the MV (no more cotacoes_tempo_real bridge)
...
-- Clean up: the unique index on cotacoes_tempo_real is no longer needed
DROP INDEX IF EXISTS idx_cotacoes_tempo_real_codigo_fundo;
