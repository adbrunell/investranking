...
- `limit` defaults to 1000 — paginate with `limit=N&offset=M` + `Prefer: count=exact`. `apiAll()` (defined per-page in `pages/analise-fii.html`) and `apiAllRadar()` (in `pages/anuncios.html`) handle this automatically
- `tipo=neq.` means `tipo != ''`
